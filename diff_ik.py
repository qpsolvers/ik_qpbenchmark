...
def find_results_file(args):
    ...
    if args.command in ["check_results", "report"]:
        results_file = (
            ...
            # f"{args.test_set_path}/results/{os.path.split(args.test_set_path)[1]}.csv"
        )
...
def load_test_set(path : str) -> TestSet:
    ...
    Args:
        path (str): path to the .py file that contains the class definition of the TestSet

    Returns:
        TestSet: _description_
    """
    import sys
    dir_path , full_name = os.path.split(path)
    name = full_name[:-3]
    sys.path.append(dir_path)  # Add directory path to system path
